...
- `src/component2/anchor_selector.py`
  ...
  - `is_valid_entity(entity: str) -> bool`
...
- `tests/component2/test_anchor_selector.py`
  - validates seed-first anchors, text fallback, degree fallback, max-K behavior, single-seed augmentation to anchor pairs, and blank-entity filtering.
- `tests/component2/test_graph_builder.py`
...
## 9) Post-M3 stability updates affecting M1 modules (2026-02-10)

- `AnchorSelector.select_anchors()` no longer exits early on a single resolved seed anchor; it now augments via text/degree fallback until at least 2 anchors when the graph permits.
- Blank entities are now explicitly excluded from graph entity extraction and fallback matching to avoid degenerate anchors (e.g., `""`).
- Regression coverage was added in `tests/component2/test_anchor_selector.py` for both behaviors.
