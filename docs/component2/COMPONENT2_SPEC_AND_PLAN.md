...
Status note (2026-02-10): this spec remains the design reference; implementation status is tracked in `docs/component2/COMPONENT2_PLAN.md`. In this repo, tasks through M3 are complete and current work starts at M4 ablations/evaluations.
...
4) If seed/entity-set anchors resolve to fewer than 2 anchors and the graph has at least 2 valid entities, keep augmenting with fallbacks until you have 2 anchors (enables pair-based connectivity diagnostics).
5) Ignore blank entities (empty/whitespace-only strings) during anchor extraction and fallback matching.
6) If too many anchors, keep the top K by:
...
AND
- `DeltaConn_bridge@k > 0`  (bridge-policy top-k on S predicts connectivity gain)

Legacy note:
- `RPI@k > τ_rpi` remains a reference diagnostic only in this repo; recovery trigger is bridge-policy-aware `DeltaConn_bridge@k`.
...
- M3 uses threshold-free evaluation by default (risk-coverage curve + AURC) and does not lock a fixed abstain threshold in code.
...
- [x] **(T1)** Find/confirm where claim entities are stored / used in retrieval in Fact-or-Fiction repo
- [x] **(T2)** Implement `AnchorSelector` with the hierarchy (A/B/C) in §4
- [x] **(T3)** Implement `GraphBuilder` for A∪C (entities as nodes, triples as typed edges)
- [x] **(T4)** Add edge features: relation id + PV features + pool id
- [x] **(T5)** Implement `MaskedGAT` (or R-GCN) backbone
- [x] **(T6)** Implement dual-stream masking (support/refute)
- [x] **(T7)** Implement pooling and classifier head
- [x] **(T8)** Implement `BridgeRescuePPR` (PPR + BridgeBonus’)
- [x] **(T9)** Implement diagnostic-triggered 1-step recovery (S→A) + re-run
- [x] **(T10)** Implement salience extraction + top-N rationale output
- [x] **(T11)** Add selective prediction (abstention) + risk–coverage evaluation
- [ ] **(T12)** Add ablations: no PV mask / no bridge rescue / no recovery

### 10.2 Tests you should write (minimum)

- [x] **(Test A)** anchors are non-empty and belong to the claim graph
- [x] **(Test B)** PPR bridge scores are stable and monotonic in rel(e)
- [x] **(Test C)** recovery actually increases connectivity on a constructed toy graph
- [x] **(Test D)** model runs end-to-end on 5 claims (CPU) without crashing
- [x] **(Test E)** rationale edge extraction returns exactly N edges and they exist in graph
...
If you are resuming this specific repo state (M3 complete), skip to M4 tasks in `docs/component2/COMPONENT2_PLAN.md` (ablations + sensitivity/evaluation). The sequence below remains the canonical implementation order for greenfield setup.
